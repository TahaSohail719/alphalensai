...
export default function PNLCalculator({
  ...
  const calculatePNL = (change: number): { pnl: number; pnlPercent: number } => {
    ...
    if (isFX) {
      ...
      const pipValue = (positionSize * 100000) / currentPrice;
      pnl = change * pipValue;
    } else {
      // Crypto/Commodities: calculate in %
      pnl = positionSize * currentPrice * (change / 100);
    }
  ...
  const calculateTPSL = () => {
    ...
    if (prefilledStopLoss) {
      const slChange = isFX 
        ? (prefilledStopLoss - prefilledEntry) * 10000 // pips
        : ((prefilledStopLoss - prefilledEntry) / prefilledEntry) * 100; // %
    ...
    prefilledTargets.forEach((target, idx) => {
      const tpChange = isFX
        ? (target - prefilledEntry) * 10000 // pips
        : ((target - prefilledEntry) / prefilledEntry) * 100; // %
